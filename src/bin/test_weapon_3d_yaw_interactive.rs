...
use sas::game::md3_pose::{
    attach_rotated_entity, axis_from_mat3, identity_axis, orientation_to_mat4, Orientation,
};
...
impl Viewer {
    async fn load(player_name: &str, weapon: Weapon) -> Result<Self, String> {
        let mut player_model = PlayerModel::load_async(player_name)
            .await
        ...
        if let Some(ref weapon_mdl) = weapon_model {
            if let Some(tx) =
                skin_loader::load_texture_file(&format!("q3-resources/{}", texture_path)).await
            {
    ...
    fn render_mesh_3d(
        ...
        for tri in &mesh.triangles {
            ...
            if i0 >= mesh.tex_coords.len()
                || i1 >= mesh.tex_coords.len()
                || i2 >= mesh.tex_coords.len()
            {
                continue;
            }
            ...
            vertices.push(Vertex {
                position: vec3(p0.x, p0.y, p0.z),
                uv: vec2(t0.coord[0], t0.coord[1]),
                color: [255, 255, 255, 255],
                normal: vec4(0.0, 0.0, 1.0, 0.0),
            });
            vertices.push(Vertex {
                position: vec3(p1.x, p1.y, p1.z),
                uv: vec2(t1.coord[0], t1.coord[1]),
                color: [255, 255, 255, 255],
                normal: vec4(0.0, 0.0, 1.0, 0.0),
            });
            vertices.push(Vertex {
                position: vec3(p2.x, p2.y, p2.z),
                uv: vec2(t2.coord[0], t2.coord[1]),
                color: [255, 255, 255, 255],
                normal: vec4(0.0, 0.0, 1.0, 0.0),
            });
            indices.push(base);
    ...
    fn render(&self) {
        ...
        let mut base_axis = axis_from_mat3(base_rot);
        ...
        if let Some(ref player) = self.player_model {
            ...
            if let Some(ref lower) = player.lower {
                ...
                if let Some(tags_for_frame) = lower.tags.get(lower_frame_idx) {
                    if let Some(torso_tag_ref) = tags_for_frame
                        .iter()
                ...
                let lower_model_m = orientation_to_mat4(&lower_orientation);
            ...
            let upper_frame_idx = self.upper_frame;
            if let Some((torso_pos, torso_axis)) = torso_tag {
                let orient = attach_rotated_entity(
                    &lower_orientation,
                    identity_axis(),
                    torso_pos,
                    torso_axis,
            ...
            if let Some(ref upper) = player.upper {
                let upper_frame_idx = upper_frame_idx.min(upper.tags.len().saturating_sub(1));
            ...
            {
                let head_orient =
                    attach_rotated_entity(&torso_orient, identity_axis(), head_pos, head_axis);
                for mesh in &head_model.meshes {
                    let mesh_name = String::from_utf8_lossy(&mesh.header.name)
                        .trim_end_matches('\0')
                        .to_string();
                    let texture = player.textures.get(&mesh_name);
                    Self::render_mesh_3d(mesh, 0, orientation_to_mat4(&head_orient), texture);
                }
            }
            ...
            {
                let weapon_orient =
                    attach_rotated_entity(&torso_orient, identity_axis(), weapon_pos, weapon_axis);
                for mesh in &weapon_model.meshes {
                    let mesh_name = String::from_utf8_lossy(&mesh.header.name)
                        .trim_end_matches('\0')
                        .to_string();
                    let texture = self.weapon_textures.get(&mesh_name);
                    Self::render_mesh_3d(mesh, 0, orientation_to_mat4(&weapon_orient), texture);
                }
            }
        }
...
async fn main() {
    ...
    let mut viewer = Viewer::load("sarge", Weapon::Plasmagun)
        .await
        .expect("load model");
    ...
    loop {
        ...
        if is_key_down(KeyCode::Minus) {
            viewer.scale = (viewer.scale - 0.5).max(1.0);
        }
        if is_key_down(KeyCode::Equal) {
            viewer.scale = (viewer.scale + 0.5).min(50.0);
        }
        ...
        if let Some(ref player) = viewer.player_model {
            if let Some(ref lower) = player.lower {
                ...
                if is_key_pressed(KeyCode::Left) {
                    viewer.lower_frame = viewer.lower_frame.saturating_sub(1).min(max_lower);
                }
                if is_key_pressed(KeyCode::Right) {
                    viewer.lower_frame = (viewer.lower_frame + 1).min(max_lower);
                }
            }
            if let Some(ref upper) = player.upper {
                let max_upper = upper.tags.len().saturating_sub(1);
                if is_key_pressed(KeyCode::Up) {
                    viewer.upper_frame = viewer.upper_frame.saturating_sub(1).min(max_upper);
                }
                if is_key_pressed(KeyCode::Down) {
                    viewer.upper_frame = (viewer.upper_frame + 1).min(max_upper);
                }
            }
        }

        while viewer.yaw > std::f32::consts::PI {
            viewer.yaw -= 2.0 * std::f32::consts::PI;
        }
        while viewer.yaw < -std::f32::consts::PI {
            viewer.yaw += 2.0 * std::f32::consts::PI;
        }
        while viewer.pitch > std::f32::consts::PI {
            viewer.pitch -= 2.0 * std::f32::consts::PI;
        }
        while viewer.pitch < -std::f32::consts::PI {
            viewer.pitch += 2.0 * std::f32::consts::PI;
        }
        while viewer.roll > std::f32::consts::PI {
            viewer.roll -= 2.0 * std::f32::consts::PI;
        }
        while viewer.roll < -std::f32::consts::PI {
            viewer.roll += 2.0 * std::f32::consts::PI;
        }
        ...
        draw_text(
            &format!("Lower Frame: {}", viewer.lower_frame),
            10.0,
            140.0,
            22.0,
            WHITE,
        ...
        draw_text(
            &format!("Upper Frame: {}", viewer.upper_frame),
            10.0,
            165.0,
            22.0,
            WHITE,
        ...
        draw_text(
            "Mouse X/Y: Yaw/Pitch | Q/E: Roll | A/D: Yaw | W/S: Pitch | -/+: Scale | R: Reset",
            10.0,
            190.0,
            18.0,
            YELLOW,
        ...
        draw_text(
            "Left/Right: Lower Frame | Up/Down: Upper Frame",
            10.0,
            215.0,
            18.0,
            YELLOW,
