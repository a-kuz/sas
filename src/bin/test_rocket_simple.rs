...
async fn main() {
    ...
    for (name, angle) in angles {
        ...
        draw_text(&format!("Angle: {:.2} rad ({:.0}°)", angle, angle.to_degrees()), 
        ...
        if let Some(mesh) = model.meshes.first() {
            ...
            draw_text(&format!("rotation = angle + PI/2 = {:.2}", angle + PI/2.0), 10.0, 130.0, 20.0, GREEN);
        ...
        draw_text(&format!("Press SPACE to save: {}", name), 10.0, 550.0, 25.0, YELLOW);
