...
async fn main() {
    ...
    for (name, pitch, yaw, roll) in test_cases {
        ...
        render_target.texture.get_texture_data().export_png(&format!("test_{}.png", name));
        println!("Saved test_{}.png", name);
    }
    
    println!("\nTesting flight angles:");
    
    let flight_angles = vec![0.0, PI/4.0, PI/2.0, 3.0*PI/4.0, PI, -PI/4.0, -PI/2.0];
    ...
    for flight_angle in flight_angles {
        ...
        draw_text(&format!("Flight: {:.0}° (vel_x={:.2}, vel_y={:.2})", 
                          flight_angle.to_degrees(), vel_x, vel_y), 
        ...
        draw_text("render_md3_mesh_with_pivot_and_yaw_ex: angle", 10.0, 70.0, 20.0, YELLOW);
        for mesh in &model.meshes {
            md3_render::render_md3_mesh_with_pivot_and_yaw_ex(
                mesh, 0,
                400.0, 300.0,
                2.5, WHITE,
                Some(&texture), Some("q3-resources/models/ammo/rocket/rocket.png"),
                false,
                flight_angle, 0.0,
                0.0, 0.0,
                0.0
