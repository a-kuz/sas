...
impl SnapshotBuffer {
    ...
    pub fn add_snapshot(&mut self, tick: u32, timestamp: f64, players: Vec<PlayerState>, projectiles: Vec<ProjectileState>) {
    ...
    pub fn interpolate_player(&self, player_id: u16, render_time: f64) -> Option<InterpolatedPlayer> {
        ...
        if render_time > latest_snap.timestamp {
            let player = latest_snap.players.iter().find(|p| p.player_id == player_id)?;
        ...
        let from_player = from_snap.players.iter().find(|p| p.player_id == player_id)?;
        ...
        Some(InterpolatedPlayer {
            position: (
                from_player.position.0 + (to_player.position.0 - from_player.position.0) * smoothed_fraction,
                from_player.position.1 + (to_player.position.1 - from_player.position.1) * smoothed_fraction,
            ),
            angle: lerp_angle(from_player.angle, to_player.angle, smoothed_fraction),
            velocity: (
                from_player.velocity.0 + (to_player.velocity.0 - from_player.velocity.0) * smoothed_fraction,
                from_player.velocity.1 + (to_player.velocity.1 - from_player.velocity.1) * smoothed_fraction,
            ),
            health: to_player.health,
            armor: to_player.armor,
            weapon: to_player.weapon,
    ...
    pub fn interpolate_projectile(&self, projectile_id: u32, render_time: f64) -> Option<InterpolatedProjectile> {
