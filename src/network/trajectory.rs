...
impl Trajectory {
    ...
    pub fn evaluate(&self, at_time: u32) -> (f32, f32) {
        ...
        match self.tr_type {
            ...
            TrajectoryType::Gravity => {
                ...
                (
                    self.tr_base_x + self.tr_delta_x * delta_time,
                    self.tr_base_y + self.tr_delta_y * delta_time + 0.5 * GRAVITY * delta_time * delta_time,
                )
            }
        }
    }
    ...
    pub fn evaluate_velocity(&self, at_time: u32) -> (f32, f32) {
        ...
        match self.tr_type {
            ...
            TrajectoryType::Linear => {
                (self.tr_delta_x, self.tr_delta_y)
            }
            TrajectoryType::Gravity => {
                const GRAVITY: f32 = 0.25 * 60.0;
                (
                    self.tr_delta_x,
                    self.tr_delta_y + GRAVITY * delta_time,
                )
            }
        }
    }
...
impl ProjectileTrajectory {
    pub fn new(id: u32, x: f32, y: f32, vel_x: f32, vel_y: f32, weapon_type: u8, owner_id: u16, spawn_time: u32) -> Self {
