...
impl NetHud {
    ...
    fn render_stats(&self, stats: &NetStats) {
        ...
        draw_rectangle(x - 5.0, y - 5.0, 310.0, 150.0, Color::from_rgba(0, 0, 0, 180));
        
        let text_color = WHITE;
        let mut line_y = y;
        
        draw_text(&format!("Ping: {} ms", stats.ping), x, line_y, 20.0, text_color);
        line_y += 20.0;
        
        draw_text(&format!("Packet Loss: {:.1}%", stats.packet_loss), x, line_y, 20.0, text_color);
        line_y += 20.0;
        
        draw_text(&format!("Incoming: {} B/s", stats.incoming_rate), x, line_y, 20.0, text_color);
        line_y += 20.0;
        
        draw_text(&format!("Outgoing: {} B/s", stats.outgoing_rate), x, line_y, 20.0, text_color);
        line_y += 20.0;
        
        draw_text(&format!("Snapshots: {}/s", stats.snapshot_rate), x, line_y, 20.0, text_color);
        line_y += 20.0;
        
        draw_text(&format!("Pred Errors: {}", stats.prediction_errors), x, line_y, 20.0, text_color);
        line_y += 20.0;
        
        draw_text(&format!("Extraps: {}", stats.extrapolations), x, line_y, 20.0, text_color);
    }
    ...
    fn render_graph(&self, stats: &NetStats) {
        ...
        draw_rectangle(x - 5.0, y - 5.0, width + 10.0, height + 10.0, Color::from_rgba(0, 0, 0, 180));
        ...
        draw_text(&format!("Ping Graph ({}ms max)", ping_data.iter().max().unwrap_or(&0)), 
            x, y - 10.0, 16.0, WHITE);
    }
    ...
    fn render_prediction_errors(&self, error: Option<&PredictionError>) {
        if let Some(err) = error {
            ...
            draw_rectangle(x - 5.0, y - 25.0, 310.0, 40.0, Color::from_rgba(0, 0, 0, 180));
            ...
            draw_text(
                &format!("Prediction Error: {:.2}px (x:{:.2}, y:{:.2})", 
