...
impl WeaponHandler {
    pub fn handle_weapon_input_multiplayer(
        ...
        if let Some(player) = game_state.players.get_mut(player_idx) {
            if input.shoot && player.refire <= 0.0 && player.weapon_switch_time <= 0.0 {
                ...
                if player.ammo[weapon_idx] >= player.weapon.ammo_per_shot()
                    || player.weapon as u8 == 0
                {
                    player.refire = player.weapon.refire_time_seconds();
                    if player.weapon as u8 > 0 {
                        player.ammo[weapon_idx] =
                            player.ammo[weapon_idx].saturating_sub(player.weapon.ammo_per_shot());
                    }
                            ...
                            && player.weapon_switch_time <= 0.0
                        {
                                ...
                                || player.weapon as u8 == 0
                            {
                                ...
                                game_state.audio_events.push(
                                    audio::events::AudioEvent::WeaponFire {
                                        weapon: player.weapon,
                                        x: player.x,
                                        has_quad: player.powerups.quad > 0,
                                    },
    ...
    pub fn handle_weapon_switching(
        ...
        if game_state.is_local_multiplayer {
                ...
                .enumerate()
            {
                ...
                if let Some(player) = game_state.players.get_mut(player_idx) {
                    Self::process_weapon_switch_simple(
                        weapon_switch,
                        player,
                        &mut game_state.audio_events,
                    );
                }
            }
        } else {
    ...
    fn process_weapon_switch_simple(
        ...
        if let Some(weapon_idx) = weapon_switch {
            ...
            if new_weapon as u8 != player.weapon as u8 {
                ...
                player.weapon_raising = true;
                player.weapon_raise_time = 0.3;
    ...
    pub fn process_weapon_fire(
        ...
        for (shoot_x, shoot_y, angle, player_id, weapon) in shoot_actions {
            ...
            game_state
                .muzzle_flashes
                .push(crate::game::muzzle::MuzzleFlash::new(
                    muzzle_x, muzzle_y, angle, weapon,
                ));
            game_state
                .lights
                .push(crate::game::light::LightPulse::from_weapon(
                    shoot_x + angle.cos() * 18.0,
                    shoot_y + angle.sin() * 18.0,
                    weapon as u8,
                ));

            Self::handle_weapon_specific_effects(
                game_state, shoot_x, shoot_y, angle, player_id, weapon,
            );
        }
    }
        ...
        match weapon {
            Weapon::RocketLauncher | Weapon::GrenadeLauncher | Weapon::Plasmagun | Weapon::BFG => {
                ...
                let (vel_x, vel_y, has_quad) = if let Some(idx) = player_idx {
                    (
                        game_state.players[idx].vel_x,
                        game_state.players[idx].vel_y,
                        game_state.players[idx].powerups.quad > 0,
                    )
                } else {
                    (0.0, 0.0, false)
                };
                ...
                let mut projectile = crate::game::projectile::Projectile::new(
                    shoot_x, shoot_y, angle, player_id, weapon, vel_x, vel_y,
                );
                if has_quad {
                    projectile.damage *= 3;
                }
            ...
            Weapon::Lightning | Weapon::Gauntlet => {
                Self::handle_lightning_gauntlet_fire(
                    game_state, shoot_x, shoot_y, angle, player_id, weapon,
                );
            }
        }
    ...
    fn handle_railgun_fire(
        ...
        game_state
            .railgun_effects
            .fire_railgun(barrel_x, barrel_y, end_x, end_y, player_color);

        let mut damage = Weapon::Railgun.damage();
        if game_state
            .players
            .iter()
            .any(|p| p.id == player_id && p.powerups.quad > 0)
        {
            damage *= 3;
        }
            ...
            if target.id != player_id && !target.dead {
                ...
                if crate::game::collision::line_rect_intersect(
                    ...
                    if target_dist < wall_dist {
                        game_state
                            .pending_hits
                            .push((i, damage, target.x, target.y, player_id));
                        game_state
                            .audio_events
                            .push(crate::audio::events::AudioEvent::RailgunHit { x: target.x });
                        break;
                    }
                }
            }
        ...
        for (idx, corpse) in game_state.corpses.iter().enumerate() {
            ...
            if crate::game::collision::line_rect_intersect(
                ...
                if target_dist < wall_dist {
                    ...
                    game_state
                        .gibs
                        .extend(crate::game::gib::spawn_gibs(gib_x, gib_y));
                    game_state.liquid_blood.add_blood(
                        gib_x,
                        gib_y,
                        3.0,
        ...
        game_state
            .bullet_holes
            .push(crate::game::hitscan::BulletHole::new(end_x, end_y));
    ...
    fn handle_shotgun_fire(
        ...
        for (start_x, start_y, ray_angle, owner, dmg) in rays {
            ...
            for step in 0..steps {
                ...
                if game_state
                    .map
                    .is_solid((check_x / 32.0) as i32, (check_y / 16.0) as i32)
                {
                    final_end_x = check_x;
                    final_end_y = check_y;
                    game_state
                        .bullet_holes
                        .push(crate::game::hitscan::BulletHole::new(check_x, check_y));
                    hit_this_ray = true;
                    break;
                }
            }
                ...
                if !hit_this_ray {
                    for (idx, corpse) in game_state.corpses.iter().enumerate() {
                        ...
                        if crate::game::collision::line_rect_intersect(
                            ...
                            game_state.weapon_hit_effects.push(
                                crate::game::weapon_hit_effect::WeaponHitEffect::new_blood(
                                    gib_x, gib_y,
                                ),
                            );
            ...
            game_state
                .debug_rays
                .push(crate::game::hitscan::DebugRay::new(
                    start_x,
                    start_y,
                    final_end_x,
                    final_end_y,
        ...
        for (idx, mut dmg) in hits {
            ...
            game_state
                .pending_hits
                .push((idx, dmg, target_x, target_y, player_id));
        }
    }
        ...
        for (start_x, start_y, ray_angle, owner, mut dmg) in rays {
            ...
            if !hit_wall {
                ...
                if let Some(idx) = hit_player_idx {
                    ...
                    game_state
                        .pending_hits
                        .push((idx, dmg, target.x, target.y, player_id));
                } else {
                    for (idx, corpse) in game_state.corpses.iter().enumerate() {
                        let hitbox_height = PLAYER_HITBOX_HEIGHT_CROUCH;
                        let hitbox_width = PLAYER_HITBOX_WIDTH;
    ...
    fn handle_lightning_gauntlet_fire(
        ...
        let is_lightning = matches!(weapon, Weapon::Lightning);

        let (end_x, end_y) = if is_lightning {
            let (hit_x, hit_y, _hits) = crate::game::lightning::fire_lightning_hitscan(
                shoot_x,
                shoot_y,
                angle,
                player_id,
            ...
            let dx = angle.cos() * range;
            let dy = angle.sin() * range;
            (shoot_x + dx, shoot_y + dy)
        };

        if is_lightning {
            let player_color = crate::game::lightning::get_player_lightning_color(player_id);
            game_state.lightning_effects.fire_lightning(
                shoot_x,
                shoot_y,
                end_x,
                end_y,
        ...
        for (i, target) in game_state.players.iter().enumerate() {
            if target.id != player_id && !target.dead {
                ...
                if dist < range {
                    ...
                    if (angle - angle_to_target).abs() < angle_diff
                        || matches!(weapon, Weapon::Gauntlet)
                    {
                        hit_idx = Some(i);
                        hit_x = target.x;
                        hit_y = target.y;
        ...
        if let Some(idx) = hit_idx {
            game_state
                .pending_hits
                .push((idx, damage, hit_x, hit_y, player_id));
