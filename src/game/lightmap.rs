...
impl Lightmap {
    ...
    fn calculate_shadow(world_x: f32, world_y: f32, light: &LightSource, map: &Map) -> f32 {
        ...
        let total_world_dist = (total_world_dx * total_world_dx + total_world_dy * total_world_dy).sqrt();
        if total_world_dist < 2.0 { return 1.0; }

        let step_x = if dir_tx > 0.0 { 1 } else { -1 };
        let step_y = if dir_ty > 0.0 { 1 } else { -1 };

        let inv_dx = if dir_tx.abs() < 1e-6 { f32::INFINITY } else { 1.0 / dir_tx.abs() };
        let inv_dy = if dir_ty.abs() < 1e-6 { f32::INFINITY } else { 1.0 / dir_ty.abs() };
        ...
        let mut t_max_x = if step_x > 0 { (1.0 - frac_x) * inv_dx } else { frac_x * inv_dx };
        let mut t_max_y = if step_y > 0 { (1.0 - frac_y) * inv_dy } else { frac_y * inv_dy };
