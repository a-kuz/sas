...
impl AwardIconCache {
    ...
    pub async fn load(&mut self) {
        self.excellent = load_texture("q3-resources/menu/medals/medal_excellent.png")
            .await
            .ok();
        self.impressive = load_texture("q3-resources/menu/medals/medal_impressive.png")
            .await
            .ok();
        self.humiliation = load_texture("q3-resources/menu/medals/medal_gauntlet.png")
            .await
            .ok();
        self.perfect = load_texture("q3-resources/menu/medals/medal_victory.png")
            .await
            .ok();
        self.accuracy = load_texture("q3-resources/menu/medals/medal_accuracy.png")
            .await
            .ok();
    }
...
impl LeadAnnouncement {
    ...
    pub fn update(
        &mut self,
        players: &[super::player::Player],
        local_player_id: Option<u16>,
        killer_id: Option<u16>,
        old_leader: Option<u16>,
        was_local_in_lead_group: bool,
    ) -> Option<&'static str> {
        ...
        let tied_count = sorted_players
            .iter()
            .filter(|p| p.frags == top_score)
            .count();
        ...
        println!(
            "    LeadAnnouncement::update: local_id={}, killer_id={:?}",
            local_id, killer_id
        );
        println!(
        ...
        println!(
            "      was_local_in_lead_group={}, is_local_in_lead_group={}",
            was_local_in_lead_group, is_local_in_lead_group
        );
        println!(
            "      was_local_sole_leader={}, is_local_leader={}, is_local_tied_for_lead={}",
            was_local_sole_leader, is_local_leader, is_local_tied_for_lead
        );
        println!(
            "      top_score={}, tied_count={}, local_score={}",
            top_score, tied_count, local_score
        );
...
impl GameResults {
    ...
    pub fn draw(
        &self,
        current_time: f32,
        model_cache: &mut super::model_cache::ModelCache,
        award_icon_cache: &AwardIconCache,
        weapon_model_cache: &super::weapon_model_cache::WeaponModelCache,
        _camera_x: f32,
        _camera_y: f32,
    ) {
        if !self.show {
        ...
        for (i, (id, name, frags, excellent, impressive)) in self.scores.iter().enumerate() {
            ...
            if let Some((_, model_name, _upper_frame, _lower_frame)) =
                self.player_models.iter().find(|m| m.0 == *id)
            {
                    ...
                    let (lower_frame, upper_frame) = if let Some(config) = &model.anim_config {
                        ...
                        if i == 0 {
                            ...
                            let torso_frame_offset = if config.torso_stand.looping_frames > 0 {
                                ...
                                torso_frame_in_anim
                                    .min(config.torso_stand.num_frames.saturating_sub(1))
                            };
                            let upper =
                                (config.torso_stand.first_frame + torso_frame_offset).min(152);

                            (lower, upper)
                        } else {
                            let (legs_anim, torso_anim) = match i {
                                1 => (&config.legs_idle, &config.torso_stand),
                                2 => (&config.both_death1, &config.both_death1),
