...
impl DeferredRenderer {
    ...
    pub fn new_with_scale(map: &Map, render_scale: f32) -> Self {
        ...
        println!(
            "[DeferredRenderer] Creating render target {}x{} (scale: {}x)",
        ...
        let lit_target = render_target_ex(
            target_w,
            target_h,
            RenderTargetParams {
                depth: false,
    ...
    pub fn apply_lighting(
        ...
        linear_lights: &[super::map::LinearLight],
        camera_x: f32,
        camera_y: f32,
        zoom: f32,
        ambient: f32,
...

        {
            ...
            material.set_texture(
                "dynamicLightData",
                self.dynamic_light_texture.as_ref().unwrap().clone(),
            );
            material.set_texture(
                "linearLightData",
                self.linear_light_texture.as_ref().unwrap().clone(),
            );
    ...
    fn apply_wasm_overlay_lighting(
        &mut self,
        map: &Map,
        all_lights: &[LightSource],
        linear_lights: &[super::map::LinearLight],
        ...
        let active_lights: Vec<LightSource> = dynamic_lights
            .iter()
            .filter(|light| {
                let dx = light.x - (camera_x + (screen_w / zoom) * 0.5);
                let dy = light.y - (camera_y + (screen_h / zoom) * 0.5);
        ...
        for bl in linear_lights.iter().take(4) {
            ...
            if hx > max_x {
                max_x = hx.min(screen_w);
            }
            if hy > max_y {
                max_y = hy.min(screen_h);
            }
        }
