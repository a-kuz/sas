...
impl TileRenderer {
    ...
    pub fn render_tiles(
        ...
        let end_x = (((camera_x + screen_width()) / TILE_WIDTH).ceil() as i32).min(map.width as i32);
        let start_y = ((camera_y / TILE_HEIGHT).floor() as i32).max(0);
        let end_y = (((camera_y + screen_height()) / TILE_HEIGHT).ceil() as i32).min(map.height as i32);
            ...
            for y in start_y..end_y {
                ...
                if tile.solid && !visited[x as usize][y as usize] {
                    ...
                    while x + block_width < end_x && 
                          ...
                          map.tiles[(x + block_width) as usize][y as usize].shader_name.as_ref() == shader_name &&
                    ...
                    while y + block_height < end_y && height_ok {
                        for bx in 0..block_width {
                            if !map.tiles[(x + bx) as usize][(y + block_height) as usize].solid ||
                               map.tiles[(x + bx) as usize][(y + block_height) as usize].texture_id != tex_id ||
                               map.tiles[(x + bx) as usize][(y + block_height) as usize].shader_name.as_ref() != shader_name ||
