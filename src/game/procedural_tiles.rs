...
pub fn render_procedural_tile_world(
    ...
    render_procedural_tile_world_ex(x, y, width, height, texture_id, neighbors, world_x, world_y, None);
}
...
pub fn render_procedural_tile_world_ex(
    ...
    let (left_tex, right_tex, top_tex, bottom_tex) = neighbor_textures.unwrap_or((None, None, None, None));
    ...
    if has_left_transition && left_same {
        render_smooth_transition_vertical(x, y, 5.0, height, texture_id, left_tex.unwrap(), world_y);
    }
    if has_right_transition && right_same {
        render_smooth_transition_vertical(x + width - 5.0, y, 5.0, height, texture_id, right_tex.unwrap(), world_y);
    }
    if has_top_transition && top_same {
        render_smooth_transition_horizontal(x, y, width, 5.0, texture_id, top_tex.unwrap(), world_x);
    }
    if has_bottom_transition && bottom_same {
        render_smooth_transition_horizontal(x, y + height - 5.0, width, 5.0, texture_id, bottom_tex.unwrap(), world_x);
    }
    
    let corner_radius = 6.0;
    
    if !bottom_same && !has_bottom_transition {
        draw_rectangle(x, y + height - 2.0, width, 2.0, darken_color(base_color, 0.6));
    }
    ...
    if !left_same && !has_left_transition {
        ...
        if side_height > 0.0 {
            ...
            draw_rectangle(x + 4.0, start_y, 2.0, side_height, darken_color(base_color, 0.68));
        }
    }
    ...
    if !right_same && !has_right_transition {
        ...
        if side_height > 0.0 {
            draw_rectangle(x + width - 6.0, start_y, 6.0, side_height, darken_color(base_color, 0.8));
            draw_rectangle(x + width - 6.0, start_y, 2.0, side_height, darken_color(base_color, 0.64));
            draw_rectangle(x + width - 2.0, start_y, 2.0, side_height, darken_color(base_color, 0.62));
        }
    }
    ...
    if !top_same && !has_top_transition {
        ...
        if is_grass(texture_id) {
            ...
            if grass_width > 5.0 {
                render_grass_top(x + grass_start_x, y, grass_width, left_same, right_same, world_x + grass_start_x, world_y);
            }
        } else {
    ...
    if !bottom_same && !right_same && !has_bottom_transition && !has_right_transition {
        draw_rounded_corner(x + width, y + height, corner_radius, false, false, edge_color);
    }
}

fn draw_rounded_corner(x: f32, y: f32, radius: f32, is_left: bool, is_top: bool, edge_color: Color) {
...
fn render_smooth_transition_horizontal(x: f32, y: f32, width: f32, height: f32, from_tex: u16, to_tex: u16, world_x: f32) {
...
fn render_smooth_transition_vertical(x: f32, y: f32, width: f32, height: f32, from_tex: u16, to_tex: u16, world_y: f32) {
    let color_from = get_base_color(from_tex);
    let color_to = get_base_color(to_tex);
...
pub fn render_procedural_tile_simple(
    ...
    if !right_same {
        draw_rectangle(x + width - 2.0, y, 2.0, height, darken_color(base_color, 0.70));
    }
        ...
        2 | 3 | 4 | 6 | 9 => {
            ...
            for i in 0..num_details {
                ...
                if seed < 0.6 {
                    ...
                    draw_rectangle(px, y, 1.0, 1.0, lighten_color(get_edge_color(texture_id), 1.1));
                }
            }
        }
        _ => {}
...
fn add_texture_detail(x: f32, y: f32, width: f32, height: f32, texture_id: u16, world_x: f32, world_y: f32) {
    let hash = |v: f32| -> f32 { ((v * 12.9898).sin() * 43758.5453).fract() };
    ...
    match texture_id {
        0 => {
            for i in 0..4 {
                ...
                draw_rectangle(x + wave_offset, yy, width - wave_offset * 2.0, 1.0, Color::from_rgba(60, 42, 28, 100));
            }
        }
        1 => {
        ...
        2 | 4 | 9 => {
            ...
            for i in 0..num_cracks {
                ...
                draw_line(cx, cy, cx, cy + crack_len, 1.0, Color::from_rgba(0, 0, 0, 30));
            }
        }
        3 | 5 | 8 => {
...
fn render_grass_top(x: f32, y: f32, width: f32, _left_neighbor: bool, _right_neighbor: bool, world_x: f32, _world_y: f32) {
    ...
    for i in 0..num_blades {
        ...
        draw_line(bx + 0.4, y - 0.4, bx + 0.4 + wind * 0.7, y - h * 0.7, 0.8, blade_color2);
    }
}
