...
pub fn move_with_collision(
    ...
    let hitbox_height = if crouch { PLAYER_HITBOX_HEIGHT_CROUCH } else { PLAYER_HITBOX_HEIGHT };
    ...
    if delta_y.abs() > 0.01 {
        ...
        if delta_y > 0.0 {
            ...
            let on_ground_test = (map.is_solid(check_x_left, check_y_feet) && !map.is_solid(check_x_left, check_y_body)) || 
                                 (map.is_solid(check_x_right, check_y_feet) && !map.is_solid(check_x_right, check_y_body));
    ...
    let on_ground = (map.is_solid(check_x_left_final, check_y_feet_final) && !map.is_solid(check_x_left_final, check_y_body_final)) || 
                    (map.is_solid(check_x_right_final, check_y_feet_final) && !map.is_solid(check_x_right_final, check_y_body_final));
...
pub fn line_rect_intersect(p1: Vec2, p2: Vec2, r_pos: Vec2, r_size: Vec2) -> bool {
    ...
    if tmin.x > tmax.x { std::mem::swap(&mut tmin.x, &mut tmax.x); }
    if tmin.y > tmax.y { std::mem::swap(&mut tmin.y, &mut tmax.y); }
