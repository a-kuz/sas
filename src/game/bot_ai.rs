...
use crate::game::map::{ItemType, Map};
...
impl BotAI {
    ...
    pub fn think(
        ...
        players: &[Player],
        map: &Map,
        projectiles: &[super::projectile::Projectile],
        ...
        let has_good_weapon =
            has_shotgun || has_grenade || has_rocket || has_rail || has_plasma || has_bfg;
        ...
        if needs_health || needs_armor || needs_weapon {
            for item in &map.items {
                ...
                let (is_useful, priority) = match item.item_type {
                    ItemType::Health25 | ItemType::Health50 | ItemType::Health100 => {
                        (needs_health, 2)
                    }
                    ItemType::Armor50 | ItemType::Armor100 => (needs_armor, 3),
                    ItemType::RocketLauncher | ItemType::Railgun | ItemType::LightningGun => {
                        (needs_weapon, 5)
                    }
                    ItemType::Plasmagun | ItemType::Shotgun | ItemType::GrenadeLauncher => {
                        (needs_weapon, 4)
                    }
                ...
                if is_useful {
                    ...
                    if best_item.is_none()
                        || priority > best_item_priority
                        || (priority == best_item_priority && dist < best_item.unwrap().2)
                    {
        ...
        if let Some((enemy, dist)) = closest_enemy {
            ...
            if let Some(nav_graph) = nav_graph {
                if !has_los
                    || self.stuck_counter > 20
                    || dy.abs() > 80.0
                    || (dx_abs < 100.0 && dy.abs() > 50.0)
                    || (dx_abs < 32.0 && dy.abs() > 40.0)
                {
            ...
            if !found_navigation_aid && (dy < -50.0 && dx_abs < 100.0) {
                ...
                if !found_navigation_aid {
                    for teleporter in &map.teleporters {
                        ...
                        let dist_to_tele =
                            (dx_to_tele * dx_to_tele + dy_to_tele * dy_to_tele).sqrt();
                ...
                if !found_navigation_aid && !has_los {
                    let on_ground =
                        map.is_solid(((bot.x) / 32.0) as i32, ((bot.y + 16.0) / 16.0) as i32);
    ...
    pub fn use_nav_graph(
            ...
            || self.current_path.len() <= 1
        {
            ...
            if let Some(start) = start_node {
                ...
                let mut candidates: Vec<(usize, f32)> = nav_graph
                    .nodes
                    .iter()
                    .enumerate()
                    .map(|(idx, node)| {
        ...
        if self.current_path.len() > 1 {
            ...
            let edge_to_next = nav_graph
                .edges
                .iter()
                .find(|e| e.from == current_node_id && e.to == next_node_id);
            ...
            let reached = if let Some(edge) = edge_to_next {
                match edge.edge_type {
                    ...
                    | super::nav_graph::NavEdgeType::JumpDown => dx_abs < 64.0 && dy > -32.0,
                    super::nav_graph::NavEdgeType::JumpGap => {
                        dx_abs < 64.0 && dy.abs() < 32.0 && !on_ground
                    }
                    super::nav_graph::NavEdgeType::JumpPad
                    | super::nav_graph::NavEdgeType::Teleport => dx_abs < 32.0 && dy.abs() < 48.0,
