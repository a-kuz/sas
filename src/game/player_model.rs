...
use crate::game::md3_pose::{
    apply_local_rotation, attach_rotated_entity, axis_from_mat3, identity_axis, Orientation,
};
...
impl PlayerModel {
    ...
    pub fn load(model_name: &str) -> Result<Self, String> {
        #[cfg(not(target_arch = "wasm32"))]
        {
            ...
            let lower = match super::md3::MD3Model::load(format!("{}/lower.md3", base_path)) {
                Ok(model) => {
                    println!(
                        "[MD3] ✓ Loaded lower.md3: {} frames, {} meshes",
            ...
            let upper = match super::md3::MD3Model::load(format!("{}/upper.md3", base_path)) {
                Ok(model) => {
                    println!(
                        "[MD3] ✓ Loaded upper.md3: {} frames, {} meshes",
                        model.header.num_bone_frames, model.header.num_meshes
                    );
                    Some(model)
                }
            ...
            let head = match super::md3::MD3Model::load(format!("{}/head.md3", base_path)) {
                Ok(model) => {
                    println!(
                        "[MD3] ✓ Loaded head.md3: {} frames, {} meshes",
                        model.header.num_bone_frames, model.header.num_meshes
                    );
                    Some(model)
                }
    ...
    pub async fn load_textures(&mut self, model_name: &str, skin_name: &str) {
        println!(
            "[MD3] Loading textures for {} with skin {}",
        ...
        for part in ["lower", "upper", "head"] {
            ...
            for skin in skins.iter() {
                if let Ok(mapping) =
                    super::skin_loader::SkinMapping::load(model_name, skin, part).await
                {
                    for (mesh_name, texture_path) in mapping.mesh_textures {
                        if let Some(texture) =
                            super::skin_loader::load_texture_file(&texture_path).await
                        {
                            println!("[Texture] ✓ Mapped {} -> {}", mesh_name, texture_path);
                            self.texture_paths
                                .insert(mesh_name.clone(), texture_path.clone());
                            ...
                            if let Some(pos) = name.find('_') {
                                if let Some(dot) = name.rfind('.') {
                                    if pos + 1 < dot {
                                        ...
                                        if let Ok(mapping) = super::skin_loader::SkinMapping::load(
                                            model_name, alt, part,
                                        )
                                        .await
                                        {
                                            for (mesh_name, texture_path) in mapping.mesh_textures {
                                                if let Some(texture) =
                                                    super::skin_loader::load_texture_file(
                                                        &texture_path,
                                                    )
                                                    .await
                                                {
                                                    println!(
                                                        "[Texture] ✓ Mapped {} -> {}",
                                                        mesh_name, texture_path
                                                    );
                                                    self.texture_paths.insert(
                                                        mesh_name.clone(),
                                                        texture_path.clone(),
                                                    );
                                                    self.textures
                                                        .insert(mesh_name.clone(), texture);

                                                    self.load_shader_textures(
                                                        &mesh_name,
                                                        &texture_path,
                                                    )
                                                    .await;
    ...
    async fn load_shader_textures(&mut self, mesh_name: &str, base_texture_path: &str) {
        ...
        if shader_name.contains("xaero_h") {
            if let Some(fire_tex) =
                super::skin_loader::load_texture_file("q3-resources/textures/sfx/firewalla.png")
                    .await
            {
            ...
            if let Some(env_tex) =
                super::skin_loader::load_texture_file("q3-resources/textures/effects/envmapbfg.png")
                    .await
            {
        ...
        if !additional_textures.is_empty() {
            self.shader_textures
                .insert(mesh_name.to_string(), additional_textures);
        }
    }
    ...
    pub fn render_simple(
        ...
        _debug_md3: bool,
        lighting_context: Option<&super::md3_render::LightingContext>,
        model_yaw_offset: f32,
        model_roll: f32,
        somersault_angle: f32,
        somersault_axis_y: bool,
        has_quad_damage: bool,
        barrel_roll: f32,
    ) {
        let draw_gun = crate::cvar::get_cvar_integer("cg_drawGun");
        let _weapon_to_render = if draw_gun == 0 { None } else { weapon_model };
        let base_y_offset = self.get_feet_height(lower_frame, scale);
        let base_y = screen_y - base_y_offset;
        let torso_offset_y = 0.0;
        let base_screen_x = screen_x;
        let base_screen_y = base_y - torso_offset_y;
        let effective_pitch = if flip_x { -pitch } else { pitch };
        let legs_yaw = if effective_pitch.abs() > 0.3 {
            let intensity = ((effective_pitch.abs() - 0.3) / 1.2).min(1.0);
            effective_pitch.signum() * intensity * 1.2
        } else {
            0.0
        };
        ...
        let effective_legs_roll = if flip_x {
            somersault_angle
        } else {
            -somersault_angle
        };
        let target_torso_roll = effective_model_roll + torso_roll_extra;
        let lower_rot = if somersault_axis_y {
            Mat3::from_rotation_z(model_yaw_offset)
                * Mat3::from_rotation_y(effective_legs_roll)
                * Mat3::from_rotation_x(0.0)
        } else {
            Mat3::from_rotation_z(model_yaw_offset)
                * Mat3::from_rotation_y(0.0)
                * Mat3::from_rotation_x(0.0)
                * Mat3::from_rotation_z(effective_legs_roll)
        };
        let mut lower_axis = axis_from_mat3(lower_rot);
        if flip_x {
            lower_axis[0] = -lower_axis[0];
        }
        for axis in &mut lower_axis {
            *axis *= scale;
        }
        ...
        let mut torso_orientation_opt = None;
        let mut head_tag = None;
        let mut weapon_tag = None;
        if let Some(ref lower) = self.lower {
            let lower_frame_idx = lower_frame.min(lower.tags.len().saturating_sub(1));
            if let Some(tags_for_frame) = lower.tags.get(lower_frame_idx) {
                if let Some(torso_tag) = tags_for_frame
                    .iter()
                    .find(|t| Self::get_tag_name(t) == "tag_torso")
                {
                    let torso_pos = vec3(
                    ...
                    let mut torso_orient = attach_rotated_entity(
                        &lower_orientation,
                        identity_axis(),
                        torso_pos,
                        torso_tag.axis,
                    );
                    let torso_rot = if somersault_axis_y {
                        Mat3::from_rotation_y(torso_yaw + effective_pitch - effective_legs_roll)
                            * Mat3::from_rotation_x(0.0)
                            * Mat3::from_rotation_z(target_torso_roll)
                    } else {
                        Mat3::from_rotation_y(torso_yaw + effective_pitch)
                            * Mat3::from_rotation_x(0.0)
                            * Mat3::from_rotation_z(target_torso_roll - effective_legs_roll)
                    };
                    apply_local_rotation(&mut torso_orient, torso_rot);
                    torso_orientation_opt = Some(torso_orient);
                }
            }
        }
        let mut torso_orientation = torso_orientation_opt.unwrap_or_else(|| {
            let mut torso = lower_orientation;
            let torso_rot = if somersault_axis_y {
                Mat3::from_rotation_y(torso_yaw + effective_pitch - effective_legs_roll)
                    * Mat3::from_rotation_x(0.0)
                    * Mat3::from_rotation_z(target_torso_roll)
            ...
            apply_local_rotation(&mut torso, torso_rot);
            torso
        });
        if let Some(ref lower) = self.lower {
            let lower_frame_idx = lower_frame.min(lower.tags.len().saturating_sub(1));
            for mesh in &lower.meshes {
                ...
                if let Some(tex) = texture {
                    super::md3_render::render_md3_mesh_with_orientation(
                        mesh,
                        safe_frame,
                        &lower_orientation,
                        base_screen_x,
                    ...
                    if has_quad_damage
                        && !Self::should_skip_quad_for_texture(texture_path.map(|s| s.as_str()))
                    {
                        super::md3_render::render_md3_mesh_with_orientation_quad(
                            mesh,
                            safe_frame,
                            &lower_orientation,
                            base_screen_x,
        ...
        if let Some(ref upper) = self.upper {
            let upper_frame_idx = upper_frame.min(upper.tags.len().saturating_sub(1));
            if let Some(tags_for_frame) = upper.tags.get(upper_frame_idx) {
                if let Some(head_tag_ref) = tags_for_frame
                    .iter()
                    .find(|t| Self::get_tag_name(t) == "tag_head")
                {
                    head_tag = Some((
                        vec3(
                ...
                if let Some(weapon_tag_ref) = tags_for_frame
                    .iter()
                    .find(|t| Self::get_tag_name(t) == "tag_weapon")
                {
                    weapon_tag = Some((
                        vec3(
        ...
        if let (Some(ref head_model), Some((head_pos, head_axis))) = (self.head.as_ref(), head_tag)
        {
            let mut head_orient =
                attach_rotated_entity(&torso_orientation, identity_axis(), head_pos, head_axis);
            ...
            let head_angle = rel.clamp(-0.7, 0.7) * 0.75;
            let head_rot = Mat3::from_rotation_x(head_angle);
            apply_local_rotation(&mut head_orient, head_rot);
        ...
        if let (Some(weapon), Some((weapon_pos, weapon_axis))) = (weapon_model, weapon_tag) {
            let mut weapon_orient =
                attach_rotated_entity(&torso_orientation, identity_axis(), weapon_pos, weapon_axis);
            let torso_current_pitch = torso_yaw + effective_pitch;
            let weapon_relative_pitch = effective_pitch - torso_current_pitch;
            let weapon_rot = Mat3::from_rotation_y(weapon_relative_pitch);
            apply_local_rotation(&mut weapon_orient, weapon_rot);
            let weapon_color = WHITE;
            if let Some(base_model) = weapon.models.get(0) {
                ...
                let mut barrel_orientation = None;
                if let Some(tags_frame0) = base_model.tags.get(0) {
                    if let Some(tag) = tags_frame0
                        .iter()
                        .find(|t| Self::get_tag_name(t) == "tag_barrel")
                    {
                        let barrel_pos = vec3(tag.position[0], tag.position[1], tag.position[2]);
                        let mut orient = attach_rotated_entity(
                            &weapon_orient,
                            identity_axis(),
                            barrel_pos,
                            tag.axis,
                        ...
                        if barrel_roll != 0.0 {
                            let spin = Mat3::from_rotation_x(barrel_roll);
                            apply_local_rotation(&mut orient, spin);
                        }
                        barrel_orientation = Some(orient);
                    }
                }
                if let Some(barrel_orient) = barrel_orientation {
    ...
    pub fn render_shadow_with_light(
        &self,
        screen_x: f32,
        screen_y: f32,
        light_sx: f32,
        ...
        if let Some(ref upper) = self.upper {
            ...
            if let Some(tags_for_frame) = upper.tags.get(safe_frame) {
                if let Some(head_tag) = tags_for_frame
                    .iter()
                    .find(|t| Self::get_tag_name(t) == "tag_head")
                {
                ...
                if let Some(weapon_tag) = tags_for_frame
                    .iter()
                    .find(|t| Self::get_tag_name(t) == "tag_weapon")
                {
    ...
    pub fn get_feet_height(&self, _lower_frame: usize, scale: f32) -> f32 {
        (20.0 - 8.0) * (scale / 1.5)
    }
    ...
    pub fn compute_frames(
        ...
        let legs_anim = if is_dead {
            legs_anim_name = "BOTH_DEATH1";
            &config.both_death1
        } else if landing_time > 0.0 {
            legs_anim_name = "LEGS_LAND";
            &config.legs_land
        } else if !on_ground {
            legs_anim_name = "LEGS_JUMP";
            &config.legs_jump
        } else if moving_backward && is_crouching {
            legs_anim_name = "LEGS_WALKCR (backward)";
            &config.legs_walkcr
        } else if moving_backward {
            legs_anim_name = "LEGS_BACK";
            &config.legs_back
        } else if is_crouching && is_walking {
            legs_anim_name = "LEGS_WALKCR";
            &config.legs_walkcr
        } else if is_crouching {
            legs_anim_name = "LEGS_IDLECR";
            &config.legs_idlecr
        } else if is_walking {
            legs_anim_name = "LEGS_RUN";
            &config.legs_run
        } else {
            legs_anim_name = "LEGS_IDLE";
            &config.legs_idle
        };

        let torso_anim_name;
        let torso_anim = if is_dead {
            torso_anim_name = "BOTH_DEATH1";
            &config.both_death1
        } else if weapon_raising && weapon_raise_time < 0.3 {
            torso_anim_name = "TORSO_RAISE";
            &config.torso_raise
        } else if is_attacking {
            torso_anim_name = "TORSO_ATTACK";
            &config.torso_attack
        } else {
            torso_anim_name = "TORSO_STAND";
            &config.torso_stand
        };
        ...
        let legs_frame_offset = if legs_anim.looping_frames > 0 && !is_dead && landing_time <= 0.0 {
            legs_frame_in_anim % legs_anim.looping_frames
        } else {
            legs_frame_in_anim.min(legs_anim.num_frames.saturating_sub(1))
        };
        let lower_frame = (legs_anim.first_frame + legs_frame_offset).min(260);

        let torso_fps = torso_anim.fps as f32;
        let torso_frame_in_anim = (new_time * torso_fps) as usize;
        let torso_frame_offset = if torso_anim.looping_frames > 0 && !is_dead && !weapon_raising {
            ...
            if !weapon.models.is_empty() {
                ...
                if !base_model.tags.is_empty() {
                        ...
                        .find(|t| Self::get_tag_name(t) == "tag_barrel")
                    {
                        ...
                        barrel_world_x =
                            weapon_origin_x + barrel_local_x * if flip_x { -1.0 } else { 1.0 };
