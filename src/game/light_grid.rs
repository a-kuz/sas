...
impl LightGrid {
    ...
    pub fn get_lights_for_screen(
        ...
        let min_cx = ((camera_x / self.cell_size).floor().max(0.0) as usize)
            .min(self.grid_width.saturating_sub(1));
        let max_cx =
            (((camera_x + screen_w) / self.cell_size).ceil() as usize).min(self.grid_width);
        let min_cy = ((camera_y / self.cell_size).floor().max(0.0) as usize)
            .min(self.grid_height.saturating_sub(1));
        let max_cy =
            (((camera_y + screen_h) / self.cell_size).ceil() as usize).min(self.grid_height);
