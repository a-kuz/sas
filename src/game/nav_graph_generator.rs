...
impl NavGraphGenerator {
    ...
    fn generate_teleporter_nodes(&mut self) {
        for teleporter in &self.map.teleporters {
            let node_id = self.graph.add_node(
                teleporter.x + teleporter.width / 2.0,
                teleporter.y + teleporter.height / 2.0,
                NavNodeType::Teleporter,
            );
    ...
    fn connect_item_nodes(&mut self) {
        ...
        for i in 0..num_nodes {
            ...
            for j in 0..num_nodes {
                ...
                if node_j.node_type != NavNodeType::Ground
                    && node_j.node_type != NavNodeType::Platform
                {
                    continue;
                }
    ...
    fn generate_fall_off_edges(&mut self) {
        ...
        for i in 0..self.graph.nodes.len() {
            ...
            let has_floor_ahead_left =
                self.is_solid_at(node_a.x - TILE_WIDTH, node_a.y + TILE_HEIGHT);
            let has_floor_ahead_right =
                self.is_solid_at(node_a.x + TILE_WIDTH, node_a.y + TILE_HEIGHT);
    ...
    fn can_fall_to_node(&self, x1: f32, y1: f32, x2: f32, y2: f32) -> bool {
        ...
        let move_speed = if dx.abs() < 16.0 {
            0.0
        } else {
            MAX_SPEED_AIR.min(dx.abs() / 60.0)
        };
    ...
    fn generate_jump_gap_edges(&mut self) {
        let num_nodes = self.graph.nodes.len();
        let existing_edges: std::collections::HashSet<(usize, usize)> = self
            .graph
            .edges
            .iter()
    ...
    fn generate_jump_pad_edges(&mut self) {
        ...
        for (idx, jump_pad) in self.map.jumppads.iter().enumerate() {
            ...
            for i in 0..self.graph.nodes.len() {
                ...
                let close_x = (node.x - jump_pad.x - jump_pad.width / 2.0).abs()
                    < jump_pad.width / 2.0 + PLAYER_HITBOX_WIDTH + GRID_SIZE;
                let close_y =
                    (node.y - jump_pad.y).abs() < PLAYER_HITBOX_HEIGHT + TILE_HEIGHT * 2.0;
            ...
            println!(
                "Jump pad at ({:.0}, {:.0}) with force ({:.1}, {:.1})",
            ...
            for target_node_idx in 0..self.graph.nodes.len() {
                ...
                for _frame in 0..500 {
                    ...
                    if (pos_x - target_node.x).abs() < tolerance_x
                        && (pos_y - target_node.y).abs() < tolerance_y
                    {
                        self.graph.add_edge(
                            jump_pad_node_id,
                            target_node_idx,
    ...
    fn generate_teleporter_edges(&mut self) {
        ...
        for (idx, teleporter) in self.map.teleporters.iter().enumerate() {
            ...
            for i in 0..self.graph.nodes.len() {
                ...
                let close_x = (node.x - teleporter.x - teleporter.width / 2.0).abs()
                    < teleporter.width / 2.0 + PLAYER_HITBOX_WIDTH + GRID_SIZE;
                let close_y = (node.y - teleporter.y - teleporter.height / 2.0).abs()
                    < teleporter.height / 2.0 + PLAYER_HITBOX_HEIGHT + TILE_HEIGHT * 2.0;
