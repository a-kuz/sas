...
impl StoryMode {
    ...
    fn create_campaign() -> Vec<StoryLevel> {
        vec![
            StoryLevel {
                ...
                briefing: "Vadrigar prison breached. Corrupted soldiers detected in sector 0.".to_string(),
            ...
            StoryLevel {
                ...
                briefing: "WARNING: Dimensional tear expanding. Boss-class entity approaching!".to_string(),
            ...
            StoryLevel {
                ...
                briefing: "CRITICAL: Multiple champion signatures! Dimensional overlords detected!".to_string(),
            ...
            StoryLevel {
                ...
                briefing: "FINAL OPERATION: Breach core located. Destroy the Void Keeper!".to_string(),
    ...
    pub fn update(&mut self, dt: f32, players: &mut Vec<Player>, _map: &Map) -> (Vec<Player>, bool) {
        ...
        let alive_enemies = players.iter().filter(|p| p.is_bot && !p.dead && !p.gibbed).count();
    ...
    pub fn render_hud(&self) {
        ...
        if self.show_briefing {
            ...
            draw_rectangle(0.0, screen_h * 0.2, screen_w, screen_h * 0.6, 
                Color::from_rgba(0, 0, 0, bg_alpha));
            
            draw_rectangle_lines(screen_w * 0.1, screen_h * 0.22, screen_w * 0.8, screen_h * 0.56, 3.0,
                Color::from_rgba(255, 150, 0, bg_alpha));
            
            let mission = format!("- MISSION {} -", self.current_level + 1);
            draw_text(&mission, screen_w / 2.0 - 100.0, screen_h * 0.32, 28.0, 
                Color::from_rgba(255, 150, 0, 255));
            
            draw_text(&level.name, screen_w / 2.0 - 200.0, screen_h * 0.40, 48.0, 
                Color::from_rgba(255, 200, 100, 255));
            
            draw_text(&level.description, screen_w / 2.0 - 150.0, screen_h * 0.48, 24.0, 
                Color::from_rgba(200, 200, 255, 255));
            
            draw_text(&level.briefing, screen_w / 2.0 - 400.0, screen_h * 0.60, 20.0, 
                Color::from_rgba(255, 255, 255, 255));
            
            let enemies_text = format!("TARGETS: {}", self.total_enemies);
            draw_text(&enemies_text, screen_w / 2.0 - 80.0, screen_h * 0.68, 18.0, 
                Color::from_rgba(200, 200, 200, 255));
                
            let timer_text = format!("Starting in {:.0}...", self.briefing_timer.ceil());
            draw_text(&timer_text, screen_w / 2.0 - 100.0, screen_h * 0.74, 20.0, 
                Color::from_rgba(255, 255, 0, 255));
            return;
        }
        
        let level_progress = format!("MISSION {}/{}: {}", 
            self.current_level + 1, self.levels.len(), level.name);
        draw_text(&level_progress, screen_w / 2.0 - 250.0, 35.0, 28.0, 
            Color::from_rgba(255, 200, 100, 255));
        ...
        let boss_count = level.enemy_spawns.iter().filter(|s| s.is_boss && !s.boss_name.is_empty()).count();
        ...
        if self.current_level == 0 && self.enemies_killed == 0 && !self.show_briefing {
            draw_text("Collect weapons and items from the arena to survive!", 
                15.0, screen_height() - 90.0, 16.0, Color::from_rgba(200, 220, 255, 220));
        }
    }
    ...
    pub fn render_exit_portal(&self, camera_x: f32, camera_y: f32) {
        ...
        if !self.level_complete {
            ...
            draw_circle(screen_x, screen_y, 15.0, Color::from_rgba(255, 255, 100, marker_alpha));
            draw_circle_lines(screen_x, screen_y, 20.0, 2.0, Color::from_rgba(255, 255, 0, marker_alpha));
            return;
        }
        ...
        draw_circle(screen_x, screen_y, radius, Color::from_rgba(100, 255, 100, 60));
        draw_circle(screen_x, screen_y, radius * 0.75, Color::from_rgba(100, 255, 255, 90));
        draw_circle(screen_x, screen_y, radius * 0.5, Color::from_rgba(255, 255, 100, 140));
        draw_circle(screen_x, screen_y, radius * 0.25, Color::from_rgba(255, 255, 255, 180));
        
        draw_circle_lines(screen_x, screen_y, radius, 4.0, Color::from_rgba(100, 255, 100, 220));
        draw_circle_lines(screen_x, screen_y, radius * 0.6, 3.0, Color::from_rgba(255, 255, 100, 200));
        ...
        let exit_text = "EXIT";
        draw_text(exit_text, screen_x - 28.0, screen_y + 7.0, 28.0, 
            Color::from_rgba(0, 0, 0, 200));
        draw_text(exit_text, screen_x - 30.0, screen_y + 5.0, 28.0, 
            Color::from_rgba(255, 255, 255, 255));
    }
}
