...
impl TileBorderRenderer {
    ...
    pub async fn load_border_textures(&mut self) {
        let border_paths = vec![
            (
                "metal_edge",
                "q3-resources/textures/base_trim/dark_tin2.png",
            ),
            (
                "concrete_edge",
                "q3-resources/textures/base_wall/concrete_dark.png",
            ),
    ...
    pub fn render_tile_borders(
        ...
        let has_left = x > 0
            && map.tiles[(x - 1) as usize][y as usize].solid
            && map.tiles[(x - 1) as usize][y as usize].texture_id != tile.texture_id;
        let has_right = x < (map.width as i32 - 1)
            && map.tiles[(x + 1) as usize][y as usize].solid
            && map.tiles[(x + 1) as usize][y as usize].texture_id != tile.texture_id;
        let has_top = y > 0
            && map.tiles[x as usize][(y - 1) as usize].solid
            && map.tiles[x as usize][(y - 1) as usize].texture_id != tile.texture_id;
        let has_bottom = y < (map.height as i32 - 1)
            && map.tiles[x as usize][(y + 1) as usize].solid
            && map.tiles[x as usize][(y + 1) as usize].texture_id != tile.texture_id;
        ...
        if has_right {
            draw_rectangle(
                screen_x + tile_width - border_width,
                screen_y,
                border_width,
                tile_height,
                border_color,
        ...
        if has_bottom {
            draw_rectangle(
                screen_x,
                screen_y + tile_height - border_width,
                tile_width,
                border_width,
                border_color,
            );
