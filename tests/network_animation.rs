...
fn test_opponent_idle_animation() {
    ...
    println!(
        "  Velocity: ({:.2}, {:.2})",
        player1_state.velocity.0, player1_state.velocity.1
    );
    println!("  On ground: {}", player1_state.on_ground);

    let velocity_magnitude =
        (player1_state.velocity.0.powi(2) + player1_state.velocity.1.powi(2)).sqrt();
    println!("  Velocity magnitude: {:.2}", velocity_magnitude);

    assert!(
        velocity_magnitude < 10.0,
        "Player 1 should be standing still (low velocity), but has velocity {:.2}",
        velocity_magnitude
    );
