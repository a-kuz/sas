...
fn test_interpolation_smoothness() {
    ...
    for frame in 0..120 {
        ...
        if let Some(interpolated) = client2.interpolate_player(player1_id, interp_time) {
            ...
            if frame % 30 == 0 {
                println!("  [Frame {}] Interpolated position: ({:.2}, {:.2})", 
    ...
    println!("Successful interpolations: {}", interpolated_positions.len());
    
    assert!(snapshot_count > 50, "Should receive many snapshots (got {})", snapshot_count);
    assert!(interpolated_positions.len() > 100, 
            "Should interpolate most frames (got {})", interpolated_positions.len());
    ...
    println!("\nMax position jump between frames: {:.2}px", max_frame_jump);
    
    assert!(max_frame_jump < 20.0, 
            "Position should not jump more than 20px between frames (max: {:.2}px)", max_frame_jump);
    ...
    assert!(total_distance > 50.0, 
            "Player should have moved significantly (moved: {:.2}px)", total_distance);
...
fn test_base_cmd_updates_regularly() {
    ...
    client.connect("Player1".to_string(), &format!("127.0.0.1:{}", PORT))
    ...
    let initial_cmd_time = initial_snapshot.players.iter()
    ...
    if !cmd_time_updates.is_empty() {
        ...
        println!("command_time changed from {} to {} (delta: {}ms)", 
                 initial_cmd_time, final_cmd_time, cmd_time_delta);
        
        assert!(cmd_time_delta > 500, 
                "command_time should update even without input (delta: {}ms)", cmd_time_delta);
    } else {
...
fn test_prediction_error_stays_low() {
    ...
    for frame in 0..120 {
        ...
        if let Some(error) = client.get_prediction().get_prediction_error() {
            ...
            if frame % 30 == 0 {
                println!("  [Frame {}] Prediction error: {:.2}px", frame, error.magnitude);
            }
        }
    }
    ...
    if !error_samples.is_empty() {
        ...
        assert!(max_error < 50.0, 
                "Max prediction error should be < 50px (got {:.2}px)", max_error);
        
        assert!(avg_error < 20.0, 
                "Average prediction error should be < 20px (got {:.2}px)", avg_error);
    }
