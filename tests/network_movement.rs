...
fn test_two_clients_movement_synchronization() {
    ...
    let initial_pos_server = server.get_player_position(player1_id)
        .expect("Player 1 should exist on server");
    println!("Player 1 initial position (server): ({:.2}, {:.2})", initial_pos_server.0, initial_pos_server.1);
    ...
    for i in 0..30 {
        ...
        if i % 5 == 0 {
            ...
            println!("  [Frame {}] Player 1 position on server: ({:.2}, {:.2})", i, pos.0, pos.1);
        }
    }
    ...
    let final_pos_server = server.get_player_position(player1_id)
        .expect("Player 1 should still exist on server");
    println!("\nPlayer 1 final position (server): ({:.2}, {:.2})", final_pos_server.0, final_pos_server.1);

    let snapshot = client2.last_snapshot().expect("Client 2 should have received snapshot");
    println!("Client 2 received snapshot with {} players", snapshot.players.len());
    
    let player1_state = snapshot.players.iter()
        .find(|p| p.player_id == player1_id)
        .expect("Client 2 should see Player 1 in snapshot");
    
    println!("Player 1 position seen by Client 2: ({:.2}, {:.2})", 
    ...
    assert!(distance_moved > 10.0, 
            "Player 1 should have moved significantly (moved: {:.2})", distance_moved);
    ...
    println!("Position difference: x={:.2}, y={:.2}", pos_diff_x, pos_diff_y);
...
fn test_bidirectional_movement_visibility() {
    ...
    let initial_snapshot_c1 = client1.last_snapshot().expect("Client 1 should have snapshot");
    let initial_snapshot_c2 = client2.last_snapshot().expect("Client 2 should have snapshot");
    
    let p1_initial_pos = initial_snapshot_c2.players.iter()
        .find(|p| p.player_id == player1_id)
        .map(|p| p.position)
        .expect("Player 1 should be visible to Client 2");
    
    let p2_initial_pos = initial_snapshot_c1.players.iter()
        .find(|p| p.player_id == player2_id)
        .map(|p| p.position)
        .expect("Player 2 should be visible to Client 1");
    
    println!("Initial state:");
    println!("  Player 1 position (seen by Client 2): ({:.2}, {:.2})", p1_initial_pos.0, p1_initial_pos.1);
    println!("  Player 2 position (seen by Client 1): ({:.2}, {:.2})", p2_initial_pos.0, p2_initial_pos.1);
    ...
    for i in 0..40 {
        ...
        if i % 10 == 0 {
            ...
            println!("  [Frame {}] Player 1: ({:.2}, {:.2}), Player 2: ({:.2}, {:.2})", 
    ...
    let final_snapshot_c1 = client1.last_snapshot().expect("Client 1 should have final snapshot");
    let final_snapshot_c2 = client2.last_snapshot().expect("Client 2 should have final snapshot");
    
    let p1_final_seen_by_c2 = final_snapshot_c2.players.iter()
        .find(|p| p.player_id == player1_id)
        .expect("Player 1 should still be visible to Client 2");
    
    let p2_final_seen_by_c1 = final_snapshot_c1.players.iter()
        .find(|p| p.player_id == player2_id)
    ...
    println!("  Player 2 position (seen by Client 1): ({:.2}, {:.2})", 
    ...
    println!("\nPlayer 1 vertical movement (jump): {:.2} units", p1_moved_y);
    
    assert!(p1_moved_y < -5.0, 
            "Player 1 should have jumped up (negative Y), but moved {:.2} units", p1_moved_y);

    let p2_moved_x = p2_final_seen_by_c1.position.0 - p2_initial_pos.0;
    println!("Player 2 horizontal movement: {:.2} units", p2_moved_x);
    
    assert!(p2_moved_x > 10.0, 
            "Player 2 should have moved right significantly, but only moved {:.2} units", p2_moved_x);
...
fn test_no_movement_should_fail_player1() {
    ...
    let initial_pos_server = server.get_player_position(player1_id)
        .expect("Player 1 should exist on server");
    println!("Player 1 settled position: ({:.2}, {:.2})", initial_pos_server.0, initial_pos_server.1);
    ...
    println!("\nPlayer 1 moved {:.2} units (should be minimal)", distance_moved);
...
fn test_no_jump_should_fail() {
    ...
    println!("Player 1 settled position: ({:.2}, {:.2})", p1_initial_pos.0, p1_initial_pos.1);
    ...
    let p1_moved_y = p1_final_seen_by_c2.position.1 - p1_initial_pos.1;
    println!("\nPlayer 1 vertical movement: {:.2} units (should be minimal)", p1_moved_y);
    
    assert!(p1_moved_y < -5.0, 
            "Player 1 should have jumped (moved vertically), but only moved {:.2} units", p1_moved_y);
}
...
fn test_player2_no_movement_should_fail() {
    ...
    println!("Player 2 settled position: ({:.2}, {:.2})", p2_initial_pos.0, p2_initial_pos.1);
    ...
    let p2_moved_x = p2_final_seen_by_c1.position.0 - p2_initial_pos.0;
    println!("\nPlayer 2 horizontal movement: {:.2} units (should be minimal)", p2_moved_x);
