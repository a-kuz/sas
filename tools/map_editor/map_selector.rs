...
use super::map;
use super::state::{TILE_HEIGHT, TILE_WIDTH};
...
impl MapSelector {
    ...
    async fn generate_preview(map_name: &str) -> Option<RenderTarget> {
        ...
        let map = match map::Map::load_from_file(map_name) {
            Ok(m) => {
                println!(
                    "[Preview] Map loaded: {}x{}, {} solid tiles",
                    m.width,
                    m.height,
        ...
        println!(
            "[Preview] Creating render target {}x{}",
        ...
        println!(
            "[Preview] Map size: {}x{} pixels, scale: {}",
            map_width_pixels, map_height_pixels, scale
        );
        ...
        for item in &map.items {
            ...
            let color = match item.item_type {
                ...
                | map::ItemType::Flight
                | map::ItemType::Haste
                | map::ItemType::Invis => Color::from_rgba(200, 100, 255, 255),
        ...
        for light in &map.lights {
            ...
            draw_circle(
                ...
                py,
                radius,
                Color::from_rgba(light.r, light.g, light.b, 220),
            );
        }
    ...
    pub fn handle_input_and_render(&mut self) -> Option<String> {
        ...
        let cards_per_row =
            ((screen_width() - padding * 2.0) / (card_width + padding)).floor() as usize;
        ...
        let is_hovering_create = mx >= create_btn_x
            && mx <= create_btn_x + create_btn_w
            && my >= create_btn_y
            && my <= create_btn_y + create_btn_h;
        ...
        let plus_color = if is_hovering_create {
            WHITE
        } else {
        ...
        draw_text(
            text,
            plus_x - text_dims.width / 2.0,
            create_btn_y + create_btn_h - 30.0,
            20.0,
            plus_color,
        );
        ...
        for card in &mut self.cards {
            ...
            draw_rectangle_lines(
                ...
                2.0,
                if is_hovering {
                    Color::from_rgba(100, 150, 220, 255)
                } else {
