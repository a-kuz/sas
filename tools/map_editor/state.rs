...
impl EditorState {
    ...
    pub async fn load_textures(&mut self) {
            ...
            match load_image(path).await {
                Ok(image) => {
                    ...
                    if loaded_count % 100 == 0 {
                        println!(
                            "[Editor] Loaded {}/{} textures...",
                            loaded_count,
                            wall_texture_paths.len()
                        );
                    }
                }
                Err(e) => {
        ...
        println!(
            "[Editor] ✓ Added {} textures to background list",
            ...
            if let Ok(_) = std::fs::metadata(&shader_file) {
                ...
                for (shader_name, tex_paths, _is_additive, has_wave) in shaders {
                    ...
                    if !frames.is_empty() && (frames.len() > 1 || has_wave) {
                        let name = format!(
                            "[Anim] {}",
                            shader_name.split('/').last().unwrap_or(&shader_name)
                        );
        ...
        if let Ok(content) = std::fs::read_to_string("tile_shaders.json") {
            if let Ok(shaders) = serde_json::from_str::<Vec<tile_shader::TileShader>>(&content) {
                println!(
                    "[Editor] Loading {} custom shaders from JSON",
                    shaders.len()
                );
                ...
                println!(
                    "[Editor] ✓ Loaded {} tile shaders",
                    self.available_shaders.len()
                );
            }
        } else {
    ...
    fn find_floor_y(&self, x: f32, y: f32) -> f32 {
        ...
        while search_y < self.map.height
            && !self
                .map
                .tiles
                .get(tile_x)
                .and_then(|col| col.get(search_y))
                .map_or(false, |t| t.solid)
        {
            search_y += 1;
        }
    ...
    fn save(&mut self) {
        ...
        let map_file =
            map_loader::MapFile::from_map(&self.map, TILE_WIDTH, TILE_HEIGHT, &self.map_name);
    ...
    pub fn handle_input(&mut self) {
        ...
        if let Some(obj) = self.selected_object.clone() {
                ...
                if is_shift {
                    if let SelectedObject::Teleporter(idx) = &obj {
                        if *idx < self.map.teleporters.len() {
                            self.map.teleporters[*idx].width =
                                (self.map.teleporters[*idx].width - size_step).max(16.0);
                        }
                    }
                } else {
                    match &obj {
            ...
            if is_key_pressed(KeyCode::Right) {
                if is_shift {
                    if let SelectedObject::Teleporter(idx) = &obj {
                        if *idx < self.map.teleporters.len() {
                            self.map.teleporters[*idx].width =
                                (self.map.teleporters[*idx].width + size_step).min(256.0);
                        }
                    }
                } else {
                    match &obj {
            ...
            if is_key_pressed(KeyCode::Up) {
                if is_shift {
                    if let SelectedObject::Teleporter(idx) = &obj {
                        if *idx < self.map.teleporters.len() {
                            self.map.teleporters[*idx].height =
                                (self.map.teleporters[*idx].height - size_step).max(16.0);
                        }
                    }
                } else {
                    match &obj {
            ...
            if is_key_pressed(KeyCode::Down) {
                if is_shift {
                    if let SelectedObject::Teleporter(idx) = &obj {
                        if *idx < self.map.teleporters.len() {
                            self.map.teleporters[*idx].height =
                                (self.map.teleporters[*idx].height + size_step).min(256.0);
                        }
                    }
                } else {
                    match &obj {
        ...
        if is_key_pressed(KeyCode::Key3) {
            self.current_tool = EditorTool::SpawnPoint;
            self.selected_object = None;
            self.selected_teleporter_index = None;
        }
        if is_key_pressed(KeyCode::Key4) {
            self.current_tool = EditorTool::Item;
            self.selected_object = None;
            self.selected_teleporter_index = None;
        }
        if is_key_pressed(KeyCode::Key5) {
            self.current_tool = EditorTool::JumpPad;
            self.selected_object = None;
            self.selected_teleporter_index = None;
        }
        ...
        if is_key_pressed(KeyCode::Key7) {
            self.current_tool = EditorTool::Light;
            self.selected_object = None;
            self.selected_teleporter_index = None;
        }
        if is_key_pressed(KeyCode::Key8) {
            self.current_tool = EditorTool::Background;
            self.selected_object = None;
            self.selected_teleporter_index = None;
        }
        ...
        self.input_handler
            .handle_texture_scale(&mut self.texture_scale, context);
        self.input_handler.handle_background_controls(
            &mut self.current_bg_alpha,
            &mut self.current_bg_scale,
            context,
        );
        ...
        if is_key_pressed(KeyCode::RightBracket) {
            if let Some(SelectedObject::Light(idx)) = &self.selected_object {
                if *idx < self.map.lights.len() {
                    self.map.lights[*idx].radius =
                        (self.map.lights[*idx].radius + 20.0).min(2000.0);
                    self.mark_lightmap_dirty();
                }
            } else if let Some(SelectedObject::JumpPad(idx)) = &self.selected_object {
                if *idx < self.map.jumppads.len() {
            ...
            if self.current_tool == EditorTool::Draw {
                if is_key_down(KeyCode::LeftControl) || is_key_down(KeyCode::RightControl) {
                    ...
                    self.current_texture =
                        (self.current_texture + 1) % self.wall_textures.len() as u16;
                }
                ...
                for light in &mut self.map.lights {
                    ...
                    if dx * dx + dy * dy < 400.0 {
                        ...
                        let current_idx = radii
                            .iter()
                            .position(|&r| (r - light.radius).abs() < 10.0)
                            .unwrap_or(2);
        ...
        if self.current_tool == EditorTool::Background {
            if is_key_pressed(KeyCode::B) {
                if is_key_down(KeyCode::LeftControl) || is_key_down(KeyCode::RightControl) {
                    ...
                    self.current_bg_texture =
                        (self.current_bg_texture + 1) % self.background_textures.len();
                }
            }
        ...
        let context = super::input::get_current_context(self.current_tool, &self.selected_object);
        if self.input_handler.handle_light_radius_scroll(
            &mut self.map.lights,
            world_x_temp,
            world_y_temp,
            context,
        ...
        if is_cmd && self.current_tool == EditorTool::Draw && self.selected_object.is_none() {
            ...
            if let Some(texture) = self.tile_textures.get(&self.current_texture) {
                ...
                if is_key_pressed(KeyCode::Left) {
                    ...
                    println!(
                        "Texture offset: ({:.1}, {:.1})",
                        self.texture_offset_x, self.texture_offset_y
                    );
                }
                if is_key_pressed(KeyCode::Right) {
                    self.texture_offset_x += offset_step;
        ...
        if is_key_pressed(KeyCode::Equal) {
            if let Some(obj) = &self.selected_object {
                match obj {
                    SelectedObject::Light(idx) => {
                        if *idx < self.map.lights.len() {
                            self.map.lights[*idx].intensity =
                                (self.map.lights[*idx].intensity + 0.1).min(10.0);
                            self.mark_lightmap_dirty();
                        }
                    }
                    SelectedObject::JumpPad(idx) => {
            ...
            if let Some(obj) = &self.selected_object {
                match obj {
                    SelectedObject::Light(idx) => {
                        if *idx < self.map.lights.len() {
                            self.map.lights[*idx].intensity =
                                (self.map.lights[*idx].intensity - 0.1).max(0.0);
                            self.mark_lightmap_dirty();
                        }
                    }
                    SelectedObject::JumpPad(idx) => {
                ...
                if x >= 0 && x < self.map.width as i32 && y >= 0 && y < self.map.height as i32 {
                    match self.current_tool {
                        EditorTool::Draw => {
                            ...
                            self.map.tiles[x as usize][y as usize].texture_id =
                                self.current_texture;
                            self.map.tiles[x as usize][y as usize].shader_name =
            ...
            match self.current_tool {
                EditorTool::Select => {
                    ...
                    if self.selected_object.is_none() {
                        for (idx, tp) in self.map.teleporters.iter().enumerate() {
                            if world_x >= tp.x
                                && world_x <= tp.x + tp.width
                                && world_y >= tp.y
                                && world_y <= tp.y + tp.height
                            {
                ...
                EditorTool::Item => {
                    let item_type = match self.current_item_type {
                        ...
                        ItemPlaceType::LightningGun => map::ItemType::LightningGun,
    ...
    pub fn render(&mut self) {
        ...
        if let Some(bg_tex) = &self.background_texture {
            ...
            let start_bg_x =
                ((self.camera_x * self.zoom / bg_w).floor() * bg_w) - self.camera_x * self.zoom;
            let start_bg_y =
                ((self.camera_y * self.zoom / bg_h).floor() * bg_h) - self.camera_y * self.zoom;
        ...
        for bg_elem in &self.map.background_elements {
            let matching_tex = self
                .background_textures
                .iter()
                .find(|t| t.base_path == bg_elem.texture_name);
            ...
            if let Some(bg_tex) = matching_tex {
                ...
                if bg_elem.additive {
                    ...
                    draw_texture_ex(
                        ...
                        DrawTextureParams {
                            dest_size: Some(vec2(
                                bg_elem.width * self.zoom,
                                bg_elem.height * self.zoom,
                            )),
                            ..Default::default()
                        },
                    );
        ...
        let end_x = (((self.camera_x + screen_width() / self.zoom) / TILE_WIDTH).ceil() as i32)
            .min(self.map.width as i32);
        let start_y = ((self.camera_y / TILE_HEIGHT).floor() as i32).max(0);
        let end_y = (((self.camera_y + screen_height() / self.zoom) / TILE_HEIGHT).ceil() as i32)
            .min(self.map.height as i32);
            ...
            for y in start_y..end_y {
                ...
                if tile.solid && !visited[x as usize][y as usize] {
                    ...
                    while x + block_width < end_x
                        ...
                        && self.map.tiles[(x + block_width) as usize][y as usize].texture_id
                            == tex_id
                    ...
                    while y + block_height < end_y && height_ok {
                        for bx in 0..block_width {
                            if !self.map.tiles[(x + bx) as usize][(y + block_height) as usize].solid
                                || self.map.tiles[(x + bx) as usize][(y + block_height) as usize]
                                    .texture_id
                                    != tex_id
                    ...
                    if let Some(ref shader_name) = tile.shader_name {
                        ...
                        let tiles_x =
                            ((block_pixel_w + start_offset_x) / scaled_tex_w).ceil() as i32 + 1;
                        let tiles_y =
                            ((block_pixel_h + start_offset_y) / scaled_tex_h).ceil() as i32 + 1;

                        for ty in 0..tiles_y {
                            for tx in 0..tiles_x {
                                let tile_x_world =
                                    world_x + tx as f32 * scaled_tex_w - start_offset_x;
                                let tile_y_world =
                                    world_y + ty as f32 * scaled_tex_h - start_offset_y;
                                ...
                                let clip_right = ((tile_x_screen + scaled_tex_w)
                                    - (screen_x + block_pixel_w))
                                    .max(0.0);
                                let clip_bottom = ((tile_y_screen + scaled_tex_h)
                                    - (screen_y + block_pixel_h))
                                    .max(0.0);
        ...
        for (idx, sp) in self.map.spawn_points.iter().enumerate() {
            ...
            let is_selected =
                matches!(&self.selected_object, Some(SelectedObject::SpawnPoint(i)) if *i == idx);
        ...
        for (idx, item) in self.map.items.iter().enumerate() {
            ...
            let is_selected =
                matches!(&self.selected_object, Some(SelectedObject::Item(i)) if *i == idx);
            ...
            if let Some(icon) = self.item_icons.get_icon(&item.item_type) {
                ...
                let color = match item.item_type {
                    map::ItemType::Health25
                    | map::ItemType::Health50
                    | map::ItemType::Health100 => RED,
        ...
        for (idx, jp) in self.map.jumppads.iter().enumerate() {
            ...
            let is_selected =
                matches!(&self.selected_object, Some(SelectedObject::JumpPad(i)) if *i == idx);
        ...
        for (idx, tp) in self.map.teleporters.iter().enumerate() {
            ...
            let is_selected_object =
                matches!(&self.selected_object, Some(SelectedObject::Teleporter(i)) if *i == idx);
            ...
            draw_line(
                screen_x + tp.width * self.zoom / 2.0,
                screen_y + tp.height * self.zoom / 2.0,
                dest_screen_x,
                dest_screen_y,
                if is_selected_teleporter || is_selected_object {
                    3.0
                } else {
                    2.0
                },
        ...
        for (idx, light) in self.map.lights.iter().enumerate() {
            ...
            let is_selected =
                matches!(&self.selected_object, Some(SelectedObject::Light(i)) if *i == idx);
            ...
            draw_circle(
                ...
                screen_y,
                4.0,
                Color::from_rgba(light.r, light.g, light.b, 255),
            );
            if light.flicker {
                draw_circle_lines(
                    screen_x,
                    ...
                    10.0,
                    1.5,
                    Color::from_rgba(255, 200, 100, 180),
                );
            }
            ...
            match self.current_tool {
                EditorTool::Draw | EditorTool::Erase => {
                    ...
                    for bx in 0..self.brush_size {
                        for by in 0..self.brush_size {
                            ...
                            if x >= 0
                                && x < self.map.width as i32
                                && y >= 0
                                && y < self.map.height as i32
                            {
                    ...
                    if let Some((start_x, start_y)) = self.line_draw_start {
                        let start_sx = ((start_x as f32 * TILE_WIDTH + TILE_WIDTH / 2.0)
                            - self.camera_x)
                            * self.zoom;
                        let start_sy = ((start_y as f32 * TILE_HEIGHT + TILE_HEIGHT / 2.0)
                            - self.camera_y)
                            * self.zoom;
                        let end_sx = ((tile_x as f32 * TILE_WIDTH + TILE_WIDTH / 2.0)
                            - self.camera_x)
                            * self.zoom;
                        let end_sy = ((tile_y as f32 * TILE_HEIGHT + TILE_HEIGHT / 2.0)
                            - self.camera_y)
                            * self.zoom;
                ...
                EditorTool::Background => {
                    ...
                    let (cursor_x, cursor_y) = if self.bg_snap_to_grid {
                        let grid_x =
                            (((world_x / 32.0).round() * 32.0) - self.camera_x) * self.zoom;
                        let grid_y =
                            (((world_y / 32.0).round() * 32.0) - self.camera_y) * self.zoom;
                        (grid_x, grid_y)
                    } else {
                        (mouse_x, mouse_y)
                    };
    ...
    fn draw_property_button(&self, x: f32, y: f32, text: &str, is_hover: bool) -> bool {
        ...
        draw_rectangle_lines(
            ...
            btn_h,
            1.0,
            Color::from_rgba(120, 130, 150, 255),
        );

        let text_dims = measure_text(text, None, 16, 1.0);
        draw_text(
            text,
            x + (btn_w - text_dims.width) / 2.0,
            y + 15.0,
            16.0,
            WHITE,
        );
    ...
    fn render_texture_picker(&mut self) {
        ...
        draw_text(
            "Click to select | Ctrl+T or ESC to close | Mouse wheel to scroll",
            picker_x + 20.0,
            picker_y + 60.0,
            16.0,
    ...
    fn render_bg_texture_picker(&mut self) {
        ...
        draw_text(
            "Click to select | Ctrl+B or ESC to close | Mouse wheel to scroll",
            picker_x + 20.0,
            picker_y + 60.0,
            16.0,
            Color::from_rgba(180, 180, 180, 255),
        ...
        let static_textures: Vec<(usize, &BackgroundTexture)> = self
            .background_textures
            .iter()
            .enumerate()
        ...
        let animated_textures: Vec<(usize, &BackgroundTexture)> = self
            .background_textures
            .iter()
            .enumerate()
        ...
        let wave_textures: Vec<(usize, &BackgroundTexture)> = self
            .background_textures
            .iter()
            .enumerate()
        ...
        if !static_textures.is_empty() {
            ...
            start_y += ((static_textures.len() + cols - 1) / cols) as f32
                * (thumb_size + padding + 18.0)
                + 20.0;
        }
        ...
        if !animated_textures.is_empty() {
            ...
            start_y += ((animated_textures.len() + cols - 1) / cols) as f32
                * (thumb_size + padding + 18.0)
                + 20.0;
        }
    ...
    fn render_shader_picker(&mut self) {
        ...
        draw_text(
            "Click to select | Ctrl+X or ESC to close",
            picker_x + 20.0,
            picker_y + 60.0,
            16.0,
            Color::from_rgba(180, 180, 180, 255),
        ...
        let none_y = start_y;
        let none_hover = mx >= picker_x + 20.0
            && mx <= picker_x + picker_w - 20.0
            && my >= none_y
            && my <= none_y + item_height;
        ...
        for (idx, shader_name) in self.available_shaders.iter().enumerate() {
            ...
            let is_selected = self
                .current_shader
                .as_ref()
                .map_or(false, |s| s == shader_name);
            let is_hover = mx >= picker_x + 20.0
                && mx <= picker_x + picker_w - 20.0
                && my >= y
                && my <= y + item_height;
                ...
                SelectedObject::Item(idx) => {
                    if let Some(item) = self.map.items.get(*idx) {
                        let type_name = match item.item_type {
                            ...
                            map::ItemType::LightningGun => "Lightning Gun",
                    ...
                    if let Some(bg) = self.map.background_elements.get(*idx) {
                        (
                            ...
                            vec![
                                ...
                                format!(
                                    "Texture: {}",
                                    bg.texture_name.split('/').last().unwrap_or("unknown")
                                ),
                            ],
                        )
                    } else {
    ...
    fn render_ui(&mut self) {
        ...
        let tool_name = match self.current_tool {
            ...
            EditorTool::Draw => {
                ...
                format!(
                    "Draw ({}) Scale: {:.2}x Offset: ({:.0},{:.0}) Brush: {}x{}{}",
                    tex_name,
                    self.texture_scale,
            ...
            EditorTool::Background => {
                ...
                format!(
                    "Background ({}) A:{:.1} S:{:.1} {} {}",
                    bg_name,
                    self.current_bg_alpha,
                    self.current_bg_scale,
                    if self.current_bg_additive {
                        "ADD"
                    } else {
                        "ALPHA"
                    },
